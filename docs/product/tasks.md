...
## 25. Phase 26: Tier Differentiation (Backend Gating) - [COMPLETED]
- [x] **Backend: 4-Tier Limit Enforcement**
    - [x] `config.py`: Added VIP/GM tier constants (Groups: 20/30/1000, Targets: 100/200/1000, Tx: 500/1000/10000)
    - [x] `auth.py`: Extracted `get_user_tier_by_email()` helper at module level — BUG-A fixed
    - [x] `portfolio_service.py`: Tier-aware limits for create_group, add_target, add_transaction — BUG-B fixed (imports restored)
    - [x] `specification.md`: Updated tier matrix (PREMIUM vs VIP differentiation)
- [x] **Bug Fixes**: BUG-A (duplicate get_me route) and BUG-B (missing imports) fixed and committed
