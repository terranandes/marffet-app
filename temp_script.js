        ...
        const addNotification = (msg) => { notifications.value.unshift({ message: msg, time: new
        ...
        const defaultSettings = { region: 'TW', gmMode: false, defaultTab: 'mars', apiKey: '', language:
        detectLanguage() };
        ...
        if (valA < valB) return sortDesc.value ? 1 : -1; if (valA> valB) return sortDesc.value ? -1 : 1;
            ...
            for (let y = startYear; y <= 2025; y++) { const cur=history.find(h=> h.year === y);
                ...
                const path = calculateWealthPath(stockId, sim.value.startYear, sim.value.principal,
                sim.value.contribution);
                ...
                if (targetIndex >= 0 && targetIndex < animationFrames.value.length) {
                    ...
                    window.updateRaceFrame(animationFrames.value[targetIndex], false); } } }; const openDetail=(stock)=>
                    {
