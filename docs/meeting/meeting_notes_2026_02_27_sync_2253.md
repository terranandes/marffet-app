...
- All 6 bugs fixed today (BUG-002 through BUG-007) are **CLOSED** and deployed to Zeabur.
...
|--------|-------------|--------|
| BUG-000-CV | Local Frontend .env.local Missing | OPEN (Workflow improvement) |
| BUG-001-CV | Zeabur AI Copilot GCP API Disabled | OPEN (Boss action required) |
| BUG-002-PL | BCR Duplicate Year Data | CLOSED |
| BUG-003-PL | Portfolio Dividend Sync NaN | CLOSED |
| BUG-004-UI | Transaction Date Picker Style | CLOSED |
| BUG-005-PL | Trend Portfolio Value Mismatch | CLOSED |
| BUG-006-PL | My Race Target Merge Name Bug | CLOSED |
| BUG-007-PL | Cash Ladder UI/UX Bugs | CLOSED |

**Open Blockers:** BUG-001-CV requires Boss to enable the Generative Language API on GCP. BUG-000-CV is a DX improvement (auto-generate `.env.local` during test workflow).
...
**Deferred / Open:**
- BUG-010-CV: Mobile Portfolio Card Click Timeout (E2E test issue)
- BUG-001-CV: AI Copilot GCP API (Boss action)
...
- Await Boss direction on BUG-001-CV (GCP API enable) and mobile UX priorities.
